...
class ShippingCompanyDrawer(CommonClosing):
...
def insert_data(shipping_company, from_date=None, to_date=None):
	fields = ["pe.name as transaction_number"]
	filters = [
		"pe.docstatus = 1",
		"pm.payment_recipient = 1",
		"pm.create_payment_entry_on_delivery = 1",
		"pe.shipping_by = 'Shipping Company'",
		"pe.shipping_source= '%s'" % shipping_company,
		"pe.added_in_cd = 0",
...
@frappe.whitelist(allow_guest=True)
def getName(doctype, user):
	stock = frappe.db.sql("""select name from `tabDrawer Balance` where user = %s;""", (user))
	return stock[0][0] if stock else ""

@frappe.whitelist(allow_guest=True)
def auto_close_sc_drawer():
	from datetime import datetime
	now = datetime.now()
	current_time = now.strftime("%H")
	
	for user in frappe.db.get_list("User",filters={'enabled':1},fields=['name']):
		if frappe.db.get_value('Cash Drawer User Table',{'type':"Shipping Company Drawer",'user':user.name,'closing_time_croped':current_time},['name']):
			shipping_company = frappe.db.get_value('Shipping Company',{'user':user.name},['name'])
			transaction = []
			for d in insert_data(shipping_company, from_date=None, to_date=None):
